...
if __name__ == "__main__":
    Tee('{}_NeuAL.txt'.format(dataset_name), 'w')
    ...
    for i in range(n):
        ...
        f1_list, f2_list, dc_list, u_list = [], [], [], []
        prob = -1
        for k in range(num_cls):
            f1_k, f2_k, dc_k = EE_forward(net1, net2, x_list[k])
            u_k = f1_k + 1 / (i+1) * f2_k
            f1_list.append(f1_k)
            f2_list.append(f2_k)
            dc_list.append(dc_k)
        ...
        if ind and (query_num < budget): 
            ...
            for k in range(num_cls):
                ...
                X2_train.append(dc_list[k].detach().cpu())
                if k == pred:
                    y1.append(torch.Tensor([reward]))
                    y2.append(torch.Tensor([reward - f1_list[k] - f2_list[k]]))
                else:
                    y1.append(torch.Tensor([1 - reward]))
                    y2.append(torch.Tensor([1 - reward - f1_list[k] - f2_list[k]]))
    ...
    np.save('./res/{}/I-NeurAL_res.npy'.format(dataset_name), regret)
